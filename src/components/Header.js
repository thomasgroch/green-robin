        ...
        return (
            <header id="header" className="header container">
                ...
                <nav>
                    ...
                    <Link key={item_idx} className={classNames('nav-link', {'active': _.get(this.props, 'pageContext.url', null) === _.get(item, 'url', null)})} to={(_.get(item, 'url', null).startsWith('#') ? (_.get(item, 'url', null)) : withPrefix(_.get(item, 'url', null)))}{...(_.get(item, 'new_window', null) ? ({target: '_blank', rel: 'noopener'}) : null)}>{_.get(item, 'label', null)}</Link>
                    ))}
                </nav>
                )}
            </header>
